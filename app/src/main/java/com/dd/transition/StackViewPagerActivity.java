 ...
 */
public class StackViewPagerActivity extends FragmentActivity {

    private TextView indicatorTv;
    private View positionView;
    private ViewPager viewPager;
    private List<StackCommonFragment> fragments = new ArrayList<>(); // 供ViewPager使用
    ...
    private void fillViewPager() {
        ...
        viewPager.setPageTransformer(false, new StackPagerTransformer(viewPager));
        // 2. viewPager添加adapter
        for (int i = 0; i < 10; i++) {
            // 预先准备10个fragment
            fragments.add(new StackCommonFragment());
        }

        viewPager.setAdapter(new FragmentStatePagerAdapter(getSupportFragmentManager()) {
            @Override
            public Fragment getItem(int position) {
                StackCommonFragment fragment = fragments.get(position % 10);
